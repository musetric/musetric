export type WhisperGpuQueue = {
  runEncoder: <T>(operation: () => Promise<T>) => Promise<T>;
...
export const createWhisperGpuQueue = (device: GPUDevice): WhisperGpuQueue => {
  const { queue } = device;
  ...
  const onUncapturedError = (event: GPUUncapturedErrorEvent): void => {
    fail(new Error(`Whisper GPU error: ${event.error.message}`));
  };
  device.addEventListener('uncapturederror', onUncapturedError);

  void device.lost.then((info) => {
    if (!released) {
      fail(new Error(`Whisper GPU device lost: ${info.message}`));
    }
  });
  ...
  const assertHealthy = (): void => {
    ...
    if (released) {
      throw new Error('Whisper GPU queue has been released');
    }
  };
  ...
  queue.writeBuffer = (buffer, offset, data, rawDataOffset, rawSize) => {
    ...
    if (
      ...
      throw new RangeError('Whisper GPU writeBuffer source range is invalid');
    }
  ...
  const runEncoder = async <T>(operation: () => Promise<T>): Promise<T> => {
    assertHealthy();
    if (active) {
      throw new Error('Whisper encoder runs must not overlap');
    }
    active = true;
    try {

...
const createFftTrigTable = (windowSize: number): Float32Array<ArrayBuffer> => {
  const table = new Float32Array(windowSize * 2);
...
export const createTrigTables = (
  ...
  rowFft: createBuffer(
    device,
    'packed-fused-tiled-r2c-row-trig-table',
    createFftTrigTable(variant.rowSize),
  ),
  columnFft: createBuffer(
    device,
    'packed-fused-tiled-r2c-column-trig-table',
    createFftTrigTable(variant.columnSize),
  ),
  fourStep: createBuffer(
    device,
    'packed-fused-tiled-r2c-four-step-trig-table',
    createFourStepTrigTable(variant),
  ),
  r2c: createBuffer(
    device,
    'packed-fused-tiled-r2c-r2c-trig-table',
